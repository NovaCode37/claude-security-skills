...
[Agent **Skills**](https://docs.claude.com/en/docs/claude-code/skills) позволяют Claude Code подгружать специализированные возможности по требованию. Этот репозиторий собирает шесть скиллов по безопасности. После установки просто попросите Claude обычным языком:
...
```mermaid
flowchart LR
    A([Вы спрашиваете<br/>обычным языком]) --> B{Claude Code<br/>выбирает скилл}
